...
function AdminUsers() {
  ...
  const mutation = useMutation({
    mutationFn: (vars: { userId: string; admin: boolean }) => toggleAdmin({ data: vars }),
    onSuccess: () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground text-ui">
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-10 pt-16 pb-32">
        <Link to="/admin" className="inline-flex items-center gap-2 text-[12px] uppercase tracking-[0.2em] text-foreground/60 hover:text-foreground transition">
        ...
        <div className="mt-6">
          ...
          <p className="mt-4 text-foreground/60 max-w-xl">Promote or demote admins. Verified accounts can sign in; unverified accounts must confirm their email first.</p>
        </div>
                  ...
                  <li key={u.id} className="grid grid-cols-[1fr_auto_auto] gap-6 items-center py-5">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-3 text-[11px] uppercase tracking-[0.2em] text-foreground/50">
                        {u.verified ? (
                          <span className="inline-flex items-center gap-1 text-accent"><CheckCircle2 className="h-3 w-3" /> Verified</span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-destructive"><XCircle className="h-3 w-3" /> Unverified</span>
                        )}
                    ...
                    >
                      {isAdmin ? (<><ShieldOff className="h-4 w-4" /> Demote</>) : (<><ShieldCheck className="h-4 w-4" /> Promote</>)}
                    </button>
                  </li>
                );
              })}
