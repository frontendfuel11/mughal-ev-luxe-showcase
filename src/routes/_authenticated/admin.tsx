...
// When on /admin exactly, it renders the catalog list.
// When on /admin/new, /admin/users, /admin/:id/edit — it renders <Outlet />.
function AdminLayout() {
  const matchRoute = useMatchRoute();
  const isExact = matchRoute({ to: "/admin", fuzzy: false });

  // If we are on a child route, just render the child via Outlet
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-10 pt-16 pb-32">
        <div className="flex items-end justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              to="/admin/users"
              className="inline-flex items-center gap-2 px-6 py-3 border border-border uppercase tracking-[0.2em] text-[12px] hover:border-accent hover:text-accent transition-colors"
            >
              <Users className="h-4 w-4" /> Users
            </Link>
            <Link
              to="/admin/new"
              className="inline-flex items-center gap-2 px-6 py-3 bg-foreground text-background uppercase tracking-[0.2em] text-[12px] hover:bg-accent hover:text-accent-foreground transition-colors"
            >
              ...
              {products.map((p) => (
                <li key={p.id} className="grid grid-cols-[80px_1fr_auto] gap-6 items-center py-5">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] uppercase tracking-[0.2em] text-foreground/50 mt-1">
                  ...
                  <div className="flex items-center gap-2">
                    <Link
                      to="/admin/$id/edit"
                      params={{ id: p.id }}
                      className="h-10 w-10 grid place-items-center border border-border hover:border-accent transition"
                    >
                      <Pencil className="h-4 w-4" />
                    </Link>
                    <button
                      onClick={() => remove(p.id)}
                      className="h-10 w-10 grid place-items-center border border-border hover:border-destructive hover:text-destructive transition"
                    >
